...
numElectrons = np.zeros((len(extraId),len(electronLevel)))
stdX         = np.zeros_like(numElectrons)
stdY         = np.zeros_like(numElectrons)
errX         = np.zeros_like(numElectrons)
errY         = np.zeros_like(numElectrons)
...
for (j, fileId) in enumerate(extraId):
    for (i, fileElectronLevel) in enumerate(electronLevel):

        if printLevel >= 1:
            print "using", j, i

        fileName = "%s%02d%s%s" % (outDir, fileElectronLevel, fileId, suffix)
        ...
        for source in sources:
            ...
            numElectrons[j][i] = fileElectronLevel
            ...
            print ("ID:%1s Electrons= %6s STDX= %4.2f STDY= %4.2f IXX= %4.2f IYY= %4.2f "
                    "IXY= %4.2f") % (j, fileElectronLevel, stdx, stdy, ixx, iyy, ixy)
    
    
# Save the output arrays to a Binary FITS File
dataTable = Table( [numElectrons, stdX, errX, stdY, errY],
                    names = ("numElectrons", "stdX", "errX", "stdY", "errY") )

dataTable.write('spotData.fits', overwrite=True)
